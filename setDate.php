...
<br><br>
	...
	{
	$date = $_POST['date'];
	$conn->query("DELETE FROM election_date WHERE year(date)=year('$date') ");
	$qry = $conn->query("INSERT INTO election_date values('$date', '$user_id')");
	if($qry)
	{
	echo '<font color="blue">."You specify the date successfully1".';
